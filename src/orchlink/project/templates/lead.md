...
4. In the visible lead Pi, prefer native `delegate_worker` when available. Its default foreground mode keeps the Pi tool call genuinely pending, streams partial broker progress inline, and returns the authoritative result at completion. Use `async: true` only for independent background work; that mode returns only a tracking handle, while the inline tree summarizes active workers. Press `F8` for the detailed panel, or use `/orchlink` as fallback; Enter or `f` follows visible output, with mouse/keyboard scrolling, End-to-live, and Tab switching.
