...
- `orch jobs --result <id>` and `orch jobs --wait <id>` retrieve final results: use `--result` when the result is already done, `--wait` when you intentionally want to block.
...
orch jobs --result TDO001  # or: orch jobs --wait TDO001
```
...
Use `--result` later only to reread or debug a completed result. If both CLI output and the lead chat show the same result, treat matching task/project IDs as one result.
