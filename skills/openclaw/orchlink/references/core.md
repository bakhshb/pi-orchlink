...
- After every async `orch send`, record the task ID in your working notes/plan and run `orch jobs --active` unless you are immediately doing independent OpenClaw-owned work.
...
For OpenClaw-as-lead, prefer `orch send --wait` for discussion because the reply prints in the terminal.
