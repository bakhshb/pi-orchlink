...
Use this reference for lead-agent coordination details.
...
- `orch lead` starts or reopens the visible Pi lead session. You usually do not need this.
- `orch work` starts or reopens the visible Pi worker session. You usually need this running. If no worker is active, ask whether to use a visible worker terminal or a human-approved background worker.
